# Help.gv.at with Watson

This application demonstrates the combination of the [Conversation](http://www.ibm.com/watson/developercloud/doc/conversation/index.shtml) and [Retrieve and Rank](http://www.ibm.com/watson/developercloud/doc/retrieve-rank/#overview) services. First, users ask questions to the Conversation service. If Conversation is not able to confidently answer, the application executes a call to Retrieve and Rank to provide the user with a list of helpful answers.
...
# How the app works
The application is designed and trained for chatting with Watson about different issues on passports in Austria.  The chat interface is on the left, and the JSON that the JavaScript code receives from the server is on the right. A user is able to ask Watson questions like a real person. If he does not know the answer (not contained in the Dialog in the Conversation Service), he will use Retrieve and Rank to find the answer.
...
1 Git clone the project `https://github.ibm.com/Markus-Wals/watson-helpgv`

2 Navigate to the `watson-helpgv` folder
...
4 The built WAR file (conversation-enhanced-0.1-SNAPSHOT.war) is in the `watson-helpgv/build/libs/` folder.
...
2 Navigate to the `watson-helpgv/src/main/resources` folder. Copy the `server.env` file. 
